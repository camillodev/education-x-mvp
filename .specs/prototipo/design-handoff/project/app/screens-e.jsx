/* Flow E — Portal do responsável (Maria) · mobile
 * Cobranças, pagar (PIX/cartão), quitar boletos vencidos, assinatura no cartão,
 * notas fiscais e notificação de negativação. */

const PORTAL_COBRANCAS = [
  { id: "p0", desc: "Mensalidade Maio/2026", valor: 450, valorAtualizado: 471.0, venc: "07/05/2026", status: "vencida", forma: "Boleto", atraso: 26 },
  { id: "p1", desc: "Mensalidade Junho/2026", valor: 450, venc: "07/06/2026", status: "avencer", forma: "PIX" },
  { id: "p2", desc: "Material didático", valor: 120, venc: "20/04/2026", status: "paga", forma: "Boleto", pagoEm: "18/04/2026", nf: "00008711" },
  { id: "p3", desc: "Mensalidade Abril/2026", valor: 450, venc: "07/04/2026", status: "paga", forma: "PIX", pagoEm: "05/04/2026", nf: "00008690" },
];
...
const FlowE = ({ exit, start = "home" }) => {
  const [view, setView] = useState(start); // home | pay | success | detail | overdue | card | cardOk
  const [sel, setSel] = useState(null);
  const [copied, setCopied] = useState(false);
  const [cartao, setCartao] = useState(null); // {last4}
  const [aceito, setAceito] = useState(false);
  const [pago, setPago] = useState({}); // id -> true (quitados nesta sessão)

  const aberta = PORTAL_COBRANCAS.find((c) => c.status === "avencer");
  const vencida = PORTAL_COBRANCAS.find((c) => c.status === "vencida" && !pago[c.id]);
  const cob = PORTAL_COBRANCAS.find((c) => c.id === sel);

  const notifs = [];
  if (vencida) notifs.push({ id: "neg", kind: "danger", icon: "alert-triangle", title: "Risco de negativação", time: "Hoje",
    body: `Sua Mensalidade de Maio está vencida há ${vencida.atraso} dias. Regularize até 10/06 para evitar a inclusão do seu nome no SPC/Serasa.`,
    cta: "Quitar e regularizar", onCta: () => { setSel(vencida.id); setView("overdue"); } });
  if (aberta) notifs.push({ id: "venc", kind: "warning", icon: "clock", title: "Mensalidade a vencer", time: "Há 1 dia",
    body: `${aberta.desc} de ${brl(aberta.valor)} vence em ${aberta.venc}.`,
    cta: "Pagar agora", onCta: () => { setSel(aberta.id); setView("pay"); } });
  notifs.push({ id: "msg1", kind: "info", icon: "message-circle", title: "Kumon Camargos", time: "2 dias atrás",
    body: "Olá! Lembrando que as aulas de reposição de Junho estão confirmadas para os sábados. Qualquer dúvida, pode chamar." });
  notifs.push({ id: "msg2", kind: "info", icon: "calendar", title: "Kumon Camargos", time: "5 dias atrás",
    body: "O boletim de progresso do João já está disponível na unidade. Passe para retirar quando puder." });
  ...
  if (view === "home") {
        ...
        <div style={{ padding: "0 16px", marginTop: -16 }}>
          ...
          {vencida && (
            <Card style={{ padding: 0, marginBottom: 16, overflow: "hidden", border: "1.5px solid var(--badge-danger-fg)" }}>
              <div style={{ background: "var(--badge-danger-bg)", padding: "12px 16px", display: "flex", alignItems: "center", gap: 9 }}>
                ...
                <span style={{ fontSize: 13.5, fontWeight: 700, color: "var(--badge-danger-fg)" }}>Risco de negativação</span>
              </div>
              <div style={{ padding: 16 }}>
                <p style={{ margin: 0, fontSize: 13.5, color: "var(--color-text-muted)", lineHeight: 1.5 }}>
                  Sua <strong style={{ color: "var(--color-text)" }}>Mensalidade de Maio</strong> está vencida há {vencida.atraso} dias. Regularize até <strong style={{ color: "var(--color-text)" }}>10/06</strong> para evitar a inclusão do seu nome no SPC/Serasa.</p>
                <Button block iconLeft="shield-check" onClick={() => { setSel(vencida.id); setView("overdue"); }} style={{ marginTop: 14 }}>Quitar e regularizar</Button>
              </div>
            </Card>
          )}
            ...
            <Card style={{ padding: 20, marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                <div>
                  ...
                  <div style={{ fontSize: 32, fontWeight: 800, letterSpacing: "-0.02em", lineHeight: 1 }}>{brl(aberta.valor)}</div>
                  <div style={{ fontSize: 13, color: "var(--color-text-subtle)", marginTop: 6 }}>{aberta.desc} · vence {aberta.venc}</div>
                </div>
                <StatusBadge status={aberta.status} />
              </div>
              <Button block size="lg" iconLeft="qr-code" onClick={() => { setSel(aberta.id); setView("pay"); }}>Pagar agora</Button>
            </Card>
          )}
          ...
          <Card style={{ padding: 16, marginBottom: 20, display: "flex", alignItems: "center", gap: 13 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              {cartao ? <>
                <div style={{ fontSize: 14, fontWeight: 600 }}>Pagamento automático ativo</div>
                <div style={{ fontSize: 12.5, color: "var(--color-text-subtle)" }}>Cartão •••• {cartao.last4} · todo mês sem você fazer nada</div>
              </> : <>
            ...
            {cartao
              ...
              : <Button variant="secondary" size="sm" iconLeft="plus" onClick={() => { setAceito(false); setView("card"); }}>Cadastrar</Button>}
          </Card>

          {/* MENSAGENS DA UNIDADE */}
          <div style={{ fontSize: 13, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--color-text-subtle)", margin: "4px 4px 10px" }}>Histórico</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10, paddingBottom: 40 }}>
            {PORTAL_COBRANCAS.map((c) => {
              const isPaid = c.status === "paga" || pago[c.id];
              const isVencida = c.status === "vencida" && !pago[c.id];
              return (
                <Card key={c.id} interactive onClick={() => { setSel(c.id); setView(isVencida ? "overdue" : "detail"); }} style={{ padding: 16, display: "flex", alignItems: "center", gap: 13 }}>
                  <span style={{ width: 42, height: 42, borderRadius: 11, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center",
                    background: isPaid ? "var(--badge-success-bg)" : isVencida ? "var(--badge-danger-bg)" : "var(--badge-warning-bg)",
                    color: isPaid ? "var(--badge-success-fg)" : isVencida ? "var(--badge-danger-fg)" : "var(--badge-warning-fg)" }}>
                    <Icon name={isPaid ? "check" : isVencida ? "alert-triangle" : "clock"} size={20} /></span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14.5, fontWeight: 600 }}>{c.desc}</div>
                    <div style={{ fontSize: 12.5, color: isVencida ? "var(--badge-danger-fg)" : "var(--color-text-subtle)" }}>
                      {isPaid ? `Pago${c.pagoEm ? " em " + c.pagoEm : ""}` : isVencida ? `Vencida há ${c.atraso} dias` : `Vence ${c.venc}`}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 15, fontWeight: 700 }}>{brl(isVencida ? c.valorAtualizado : c.valor)}</div>
                    {c.nf && <div style={{ fontSize: 11.5, color: "var(--color-primary)", fontWeight: 600, marginTop: 2 }}>NF disponível</div>}
                  </div>
                </Card>
              );
            })}
    ...
    // Quitar boleto vencido — valor atualizado com multa + juros
    content = (
      <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", background: "var(--color-bg)" }}>
        <PtBack onBack={() => setView("home")} title="Quitar boleto vencido" />
        <div style={{ flex: 1, padding: 18 }}>
          <Card style={{ padding: 20, marginBottom: 14 }}>
            <div className="label" style={{ marginBottom: 8 }}>Valor atualizado</div>
            <div style={{ fontSize: 34, fontWeight: 800, letterSpacing: "-0.02em", color: "var(--badge-danger-fg)", lineHeight: 1 }}>{brl(vencida.valorAtualizado)}</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 7, marginTop: 16 }}>
              {[["Valor original", brl(vencida.valor)], ["Multa (2%)", brl(vencida.valor * 0.02)], ["Juros (1% a.m.)", brl(vencida.valorAtualizado - vencida.valor - vencida.valor * 0.02)]].map(([k, v]) => (
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, paddingTop: 9, borderTop: "1px solid var(--color-border-muted)", marginTop: 3 }}>
                <span style={{ fontWeight: 700 }}>Total a pagar</span><span style={{ fontWeight: 800, color: "var(--badge-danger-fg)" }}>{brl(vencida.valorAtualizado)}</span></div>
            </div>
          </Card>
        ...
        <div style={{ padding: "12px 18px 44px", borderTop: "1px solid var(--color-border-muted)", display: "flex", flexDirection: "column", gap: 10 }}>
          <Button block size="lg" iconLeft="qr-code" onClick={() => { setSel(vencida.id); setView("pay"); }}>Pagar com PIX</Button>
          <Button block size="lg" variant="secondary" iconLeft="check-circle" onClick={() => { setPago((p) => ({ ...p, [vencida.id]: true })); setView("success"); }}>Já paguei o boleto</Button>
        </div>
      </div>
    );
  } else if (view === "card") {
    // Cadastrar cartão para assinatura + aceite de termos
    content = (
      <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", background: "var(--color-bg)" }}>
        <PtBack onBack={() => setView("home")} title="Pagamento automático" />
        <div style={{ flex: 1, padding: 18 }}>
          ...
          <div style={{ borderRadius: 16, padding: 20, background: "linear-gradient(120deg, var(--color-primary), var(--color-hero-to))", color: "#fff", marginBottom: 20, minHeight: 130, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.1em" }}>EducationX</span>
            </div>
            <div style={{ fontSize: 17, fontFamily: "var(--font-mono)", letterSpacing: "0.12em" }}>•••• •••• •••• 4242</div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, opacity: 0.9 }}><span>MARIA SILVA</span><span>12/29</span></div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <Field label="Número do cartão"><Input defaultValue="4242 4242 4242 4242" inputMode="numeric" leadingIcon="credit-card" /></Field>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <Field label="Validade"><Input defaultValue="12/29" inputMode="numeric" /></Field>
              <Field label="CVV"><Input defaultValue="123" inputMode="numeric" /></Field>
            </div>
            <Field label="Nome no cartão"><Input defaultValue="Maria Silva" /></Field>
            ...
            <div style={{ display: "flex", gap: 9, fontSize: 12, color: "var(--color-text-subtle)", lineHeight: 1.5 }}>
              ...
              <span>Ao pagar no cartão, a escola cobra <strong>2,99%</strong> de taxa adicional sobre a mensalidade. No PIX e boleto, nenhuma taxa extra.</span>
            </div>
          </div>
        </div>
        <div style={{ padding: "12px 18px 44px", borderTop: "1px solid var(--color-border-muted)" }}>
          <Button block size="lg" disabled={!aceito} iconLeft="lock" onClick={() => { setCartao({ last4: "4242" }); setView("cardOk"); }}>Salvar cartão</Button>
        </div>
      </div>
    );
  } else if (view === "cardOk") {
      ...
      <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", background: "var(--color-bg)" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 24px", textAlign: "center" }}>
          ...
          <p style={{ margin: "14px 0 0", fontSize: 15, color: "var(--color-text-muted)", lineHeight: 1.55 }}>
            Pagamento automático ativado no cartão <strong style={{ color: "var(--color-text)" }}>•••• 4242</strong>. As próximas mensalidades serão pagas sozinhas — você ainda recebe a nota fiscal de cada uma.</p>
        </div>
    ...
    const alvo = cob || aberta;
    const valorPagar = alvo.status === "vencida" ? alvo.valorAtualizado : alvo.valor;
    content = (
      <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", background: "var(--color-bg)" }}>
        <PtBack onBack={() => setView(alvo.status === "vencida" ? "overdue" : "home")} title="Pagar com PIX" />
        <div style={{ flex: 1, padding: "8px 20px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
          ...
          <QrCode size={190} />
          <div style={{ fontSize: 13, color: "var(--color-text-subtle)", margin: "14px 0 20px" }}>Abra o app do banco e escaneie</div>
        ...
        <div style={{ padding: "12px 18px 44px", borderTop: "1px solid var(--color-border-muted)" }}>
          <Button block size="lg" iconLeft="check-circle" onClick={() => { if (alvo.status === "vencida") setPago((p) => ({ ...p, [alvo.id]: true })); setView("success"); }}>Já paguei</Button>
    ...
    const wasVencida = cob && cob.status === "vencida";
    content = (
      <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", background: "var(--color-bg)" }}>
        <PtBrand />
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 24px", textAlign: "center" }}>
          ...
          <p style={{ margin: "14px 0 0", fontSize: 15, color: "var(--color-text-muted)", lineHeight: 1.55 }}>
            {wasVencida
              ? <>Sua situação foi <strong style={{ color: "var(--color-success)" }}>regularizada</strong> e a negativação cancelada. A nota fiscal fica disponível aqui em instantes.</>
              : <>Recebemos o pagamento da mensalidade de <strong style={{ color: "var(--color-text)" }}>João</strong>. A nota fiscal fica disponível aqui em instantes.</>}</p>
        </div>
        <div style={{ padding: "12px 18px 44px" }}>
          <Button block size="lg" iconLeft="home" onClick={() => setView("home")}>Voltar ao início</Button>
        </div>
    ...
    // detail
    const isPaid = cob.status === "paga" || pago[cob.id];
    content = (
      <div style={{ minHeight: "100%", background: "var(--color-surface)" }}>
        <PtBack onBack={() => setView("home")} title="Detalhe" />
        <div style={{ padding: 16 }}>
          <Card style={{ padding: 22, marginBottom: 14, textAlign: "center" }}>
            ...
            <div style={{ marginTop: 14 }}><StatusBadge status={isPaid ? "paga" : cob.status} /></div>
          </Card>
          <Card style={{ padding: 0, overflow: "hidden", marginBottom: 14 }}>
            {[["Forma", cob.forma], [isPaid ? "Pago em" : "Vence em", isPaid ? cob.pagoEm : cob.venc], ["Cobrança", `#${cob.id.toUpperCase()}`]].map(([k, v], i, a) => (
          ...
          {isPaid ? (
            ...
            <Button block size="lg" iconLeft="qr-code" onClick={() => setView("pay")}>Pagar agora</Button>
          )}
        </div>
      </div>
    );
