...
## Arquivos-alvo
- **Novos:** `src/app/(app)/escolas/page.tsx` · `src/app/(app)/escolas/[unitId]/page.tsx` · `src/app/api/escolas/route.ts` (GET) · `src/app/api/escolas/[unitId]/route.ts` (GET+PATCH) · `src/hooks/use-schools.ts` · `src/lib/errors/handle.ts` · `e2e/escolas.spec.ts` · `tests/integration/update-school.integration.test.ts` · `tests/integration/_setup.ts` (helper transação/rollback) · `vitest.integration.config.ts`
