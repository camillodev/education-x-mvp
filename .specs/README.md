...
1. **`Enrollment` — dono `mvp-02`.** Usa `EnrollmentPlan` + `agreedPriceCents`/`finalPriceCents` + desconto em 3 campos + `EnrollmentStatus`. A `mvp-03` só referencia e acrescenta `isFirstChargeDone`, relação `invoices`.
