...
export function usePendingEnrollments() {
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error(
          `[usePendingEnrollments] GET /api/enrollments/pending falhou (${res.status} ${body.code ?? ''}):`,
  ...
  const approve = useCallback(
    async (guardianId: string) => {
      const res = await fetch(`/api/enrollments/${guardianId}/approve`, { method: 'POST' })
      if (!res.ok) {
  ...
  const reject = useCallback(
    async (guardianId: string) => {
      const res = await fetch(`/api/enrollments/${guardianId}/reject`, { method: 'POST' })
      if (!res.ok) {
        const body = (await res.json().catch(() => ({}))) as { error?: string }
